...
func hashNamed(t *types.Named) uint32 {
	// Not safe with a copying GC; objects may move.
	n := reflect.ValueOf(t.Obj()).Pointer()
...
func (h Hasher) hashTuple(tuple *types.Tuple) uint32 {
	...
	for i := 0; i < n; i++ {
		...
		hash = (hash<<5 | hash>>27) + 3*h.hashFor(tuple.At(i).Type())
	}
	return hash
}

func (h Hasher) hashVar(va *types.Var) uint32 {
	var hash uint32 = 0
	if va != nil {
		hash = h.Hash(va.Type())
	}
