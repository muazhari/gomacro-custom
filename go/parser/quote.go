...
func (p *parser) parseQuote() ast.Expr {
	...
	op := p.tok
	opPos := p.pos
	opName := etoken.String(op) // use the actual name QUOTE/QUASIQUOTE/UNQUOTE/UNQUOTE_SPLICE even if we found ~' ~` ~, ~,@
	p.next()
...
func (p *parser) parseStmtListQuoted() (list []ast.Stmt) {
	...
	for p.tok != token.RBRACE && p.tok != token.EOF {
		if p.tok == token.PACKAGE {
			stmt = &ast.DeclStmt{Decl: p.parsePackage()}
		} else if p.tok == etoken.TYPECASE {
...
func (p *parser) parseExprBlock() ast.Expr {
	...
	return &ast.UnaryExpr{OpPos: pos, Op: etoken.MACRO, X: fun}
}
